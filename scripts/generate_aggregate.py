...
Usage:
    python3 scripts/generate_aggregate.py /path/to/repo [more/repos ...] > aggregate.csv
    python3 scripts/generate_aggregate.py .  --since 2024-01-01 --output aggregate.csv
"""
import argparse
import csv
import subprocess
...
def parse_commits(output):
    """Yield (month, author, insertions, deletions, files_changed, subject_length) per commit."""
    for chunk in output.split(RS):
        if not chunk.strip():
            continue
        lines = chunk.split('\n')
        _commit_hash, month, author, subject = lines[0].split(FS)
        ...
        for line in lines[1:]:
            ...
            files_changed += 1
        yield month, author, insertions, deletions, files_changed, len(subject)


def new_month_bucket():
    return {
        'commits': 0, 'insertions': 0, 'deletions': 0, 'msg_chars': 0, 'files_changed': 0,
        'repos': set(), 'authors': set(),
    }
...
def main():
    ...
    monthly = defaultdict(new_month_bucket)
    ...
    for repo in args.repos:
        ...
        for month, author, insertions, deletions, files_changed, msg_len in parse_commits(output):
            m = monthly[month]
            m['commits'] += 1
    ...
    for month in sorted(monthly):
        ...
        churn_ratio = round(d['deletions'] / d['insertions'], 2) if d['insertions'] else 0
        writer.writerow([
            ...
            len(d['authors']), net_lines, churn_ratio, d['files_changed'],
            round(d['files_changed'] / c, 1) if c else 0,
        ])
    if args.output:
        out.close()
